...
use crate::protocol::types::{ChatRequest, Message, MessageContent, MessageRole, ResponseFormat};
...
impl LossinessReason {
    ...
    pub fn as_str(&self) -> String {
        match self {
            ...
            Self::ConsecutiveSameRoleNotSupported => {
                "messages.consecutive_same_role.unsupported".to_string()
            }
...
impl TransformationEngine {
    ...
    pub fn transform_request(&self, request: ChatRequest) -> TransformResult {
        ...
        if transformed.temperature.is_some() && !target_caps.supports_temperature {
            ...
            lossy = true;
            reasons
                .push(LossinessReason::ParameterNotSupported("temperature".to_string()).as_str());
        }
        ...
        if !target_caps.supports_consecutive_same_role
            && self.has_consecutive_same_role(&transformed)
        {
        ...
        if lossy {
            transformed
                .metadata
                .insert("lossy".to_string(), json!(true));
            transformed
                .metadata
                .insert("lossy_reasons".to_string(), json!(reasons.clone()));
        }
                    ...
                    if !system_content.is_empty() {
                        ...
                        match &message.content {
                            MessageContent::Text(user_text) => {
                                new_message.content = MessageContent::Text(format!(
                                    "{}\n\n{}",
                            ...
                            MessageContent::Parts(_) => {
                                ...
                                additional_reasons
                                    .push("content.parts.degraded_for_merge".to_string());
                                new_message.content = MessageContent::Text(format!(
