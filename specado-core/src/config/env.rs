...
pub fn interpolate_config_env_vars(config: &mut super::schema::SpecadoConfig) -> Result<(), ConfigError> {
    let env_var_pattern = Regex::new(r"\$\{([A-Z_][A-Z0-9_]*)\}").unwrap();
    
    // Interpolate provider API keys
    for provider in &mut config.providers {
        if env_var_pattern.is_match(&provider.api_key) {
            provider.api_key = interpolate_single_value(&provider.api_key)?;
        }
...
fn interpolate_single_value(value: &str) -> Result<String, ConfigError> {
    ...
    if let Some(cap) = env_var_pattern.captures(value) {
        ...
        match env::var(var_name) {
            Ok(env_value) => {
                Ok(value.replace(&cap[0], &env_value))
            }
            Err(_) => {
