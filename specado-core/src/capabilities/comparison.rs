...
impl CapabilityComparison {
    ...
    fn compare_features(
        ...
        if source.features.function_calling && !target.features.function_calling {
            ...
            lossiness.push(LossinessType::MissingFeature(
        ...
        if source.features.json_mode && !target.features.json_mode {
            ...
            details.push(
                "Target provider does not support JSON mode for structured output".to_string(),
            );
        }
        ...
        if source.features.tool_use && !target.features.tool_use {
            if !target.features.function_calling {
                ...
                details.push(
                    "Target provider does not support tool use or function calling".to_string(),
                );
            }
        }
    }
    ...
    fn compare_parameters(
        ...
        if source.parameters.temperature.supported && !target.parameters.temperature.supported {
            lossiness.push(LossinessType::ConstrainedParameter(
    ...
    fn generate_recommendations(
        ...
        let has_function_calling = lossiness_types
            .iter()
        ...
        let has_token_limit = lossiness_types
            .iter()
            .any(|t| matches!(t, LossinessType::TokenLimitExceeded));

        if has_token_limit {
            recommendations.push(
                "Consider chunking the input or using a model with larger context window"
                    .to_string(),
            );
        }

        let has_modality_loss = lossiness_types
            .iter()
            .any(|t| matches!(t, LossinessType::MissingModality(_)));

        if has_modality_loss {
            recommendations.push(
                "Consider preprocessing multimodal inputs or using a multimodal-capable provider"
                    .to_string(),
            );
        }
    }
    ...
    fn determine_severity(lossiness_types: &[LossinessType]) -> LossinessSeverity {
        ...
        for loss_type in lossiness_types {
            let severity = match loss_type {
                LossinessType::MissingFeature(f) => match f.as_str() {
...
mod tests {
    ...
    fn test_no_lossiness() {
        ...
        assert_eq!(
            comparison.lossiness_report.severity,
    ...
    fn test_constraint_lossiness() {
        ...
        assert!(comparison
            .lossiness_report
            .lossiness_types
            .contains(&LossinessType::TokenLimitExceeded));
    }
}
