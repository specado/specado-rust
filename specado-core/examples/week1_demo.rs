...
fn main() {
    ...
    println!(
        "  - System Message: \"{}\"",
        if let specado_core::protocol::types::MessageContent::Text(text) =
    ...
    println!(
        "  - JSON Mode: {}",
        if result.transformed.response_format.is_some() {
            "Enabled"
        } else {
            "Removed"
        }
    ...
    println!(
        "Original: {} messages with consecutive user messages",
        request2.messages.len()
    );
    let result2 = transform_request(request2, "anthropic");

    println!(
        "After transformation: {} messages",
        result2.transformed.messages.len()
    );
