...
use specado_core::capabilities::{
    Capability, ControlParameters, ModelFeatures, ParameterSupport, ProviderManifest,
};
...
fn main() {
    ...
    if let Some(gpt4) = openai.get_model_capabilities("gpt-4-turbo") {
        ...
        println!(
            "  - Max context window: {:?}",
            gpt4.constraints.tokens.max_context_window
        );
    }
    ...
    if let Some(claude) = anthropic.get_model_capabilities("claude-3-opus") {
        ...
        println!(
            "  - Max context window: {:?}",
            claude.constraints.tokens.max_context_window
        );
    }
    ...
    println!(
        ...
        comparison.lossiness_report.is_lossy
    );
    ...
    println!(
        "  - Temperature range: {:?} - {:?}",
        custom.parameters.temperature.min, custom.parameters.temperature.max
    );
