...
use specado_core::providers::{
    RetryPolicy, RetryExecutor, ErrorMapper, ProviderError,
    RoutingBuilder, RoutingStrategy, OpenAIProvider, AnthropicProvider,
};
...
async fn test_retry_executor_non_retryable_error() {
    ...
    let result = executor.execute(|| async {
        Err::<String, ProviderError>(ProviderError::AuthenticationError)
    }).await;
...
fn test_error_mapper_http_status_codes() {
    ...
    assert!(matches!(error, ProviderError::ServerError { status_code: 500, .. }));
    
    let error = ErrorMapper::from_status_code(503, None);
    assert!(matches!(error, ProviderError::ServerError { status_code: 503, .. }));
}
