...
fn test_capability_comparison_openai_to_anthropic() {
    ...
    let has_function_calling_loss = comparison
        .lossiness_report
        .lossiness_types
        .iter()
...
fn test_multimodal_support() {
    ...
    assert!(gpt4_turbo
        .modalities
        .supports_input(&modality::Modality::Text));
    assert!(gpt4_turbo
        .modalities
        .supports_input(&modality::Modality::Image));
...
fn test_custom_modality() {
    ...
    support
        .input
        .insert(modality::Modality::Custom("3D".to_string()));
...
fn test_authentication_requirements() {
    let openai = provider_manifest::ProviderManifest::openai();
    assert_eq!(
        openai.authentication.auth_type,
        provider_manifest::AuthType::ApiKey
    );
    assert!(openai
        .authentication
        .required_headers
    ...
    let anthropic = provider_manifest::ProviderManifest::anthropic();
    assert_eq!(
        anthropic.authentication.auth_type,
        provider_manifest::AuthType::ApiKey
    );
    assert!(anthropic
        .authentication
...
fn test_endpoint_configuration() {
    ...
    assert_eq!(
        openai.endpoints.chat_endpoint,
    ...
    assert_eq!(
        anthropic.endpoints.chat_endpoint,
...
fn test_extensions_and_experimental_features() {
    ...
    assert!(capability
        .extensions
        .custom
        .contains_key("provider_specific"));
}
