...
fn week_1_demo_value_proposition() {
    ...
    for provider in providers {
        ...
        println!("  Final message count: {}", result.transformed.messages.len());
        println!("  Metadata added: {:?}", result.transformed.metadata.keys().collect::<Vec<_>>());
        println!();
    }
...
fn week_1_demo_edge_cases() {
    ...
    assert_eq!(result.transformed.messages.len(), 1);
    assert_eq!(result.transformed.messages[0].role, specado_core::protocol::types::MessageRole::User);
