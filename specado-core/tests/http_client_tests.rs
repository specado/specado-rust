...
use specado_core::http::{HttpExecutor, RequestOptions, CallKind};
use specado_core::http::client::HttpClient;
use specado_core::protocol::types::{ChatRequest, ChatResponse, Message, MessageRole, MessageContent, ResponseChoice};
...
async fn test_success_json_response() {
    ...
    assert!(result.is_ok(), "Expected successful response, got: {:?}", result);
...
async fn test_401_unauthorized() {
    ...
    assert!(matches!(result.unwrap_err(), ProviderError::AuthenticationError));
}
