...
use specado_core::providers::{
    AnthropicProvider, OpenAIProvider, PrimaryWithFallbacks, ProviderError, ProviderType,
...
async fn test_primary_success_no_fallback() {
    ...
    let request = ChatRequest::new("gpt-4", vec![Message::user("Hello, world!")]);
...
async fn test_metadata_tracking_disabled() {
    ...
    let request = ChatRequest::new("gpt-4", vec![Message::user("Test without metadata")]);
...
async fn test_transform_metadata_preservation() {
    ...
    assert!(transform
        .reasons
        .contains(&"system_role.merged".to_string()));

    // Check metadata includes transformation info
    assert_eq!(
        result
            .metadata
            .get("transformation_lossy")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
...
async fn test_openai_to_anthropic_transformation_integration() {
    ...
    assert!(
        result.provider_errors.contains_key("openai"),
        "Should record OpenAI provider error"
    );
    let openai_error = result.provider_errors.get("openai").unwrap();
    assert!(
        openai_error.contains("timeout") || openai_error.contains("Timeout"),
        "Should record timeout error from OpenAI provider"
    );

    // ========== PROVE TRANSFORMATION PIPELINE WORKS ==========
    assert!(
        result.transform_result.is_some(),
    ...
    assert_eq!(
        result
            ...
            .and_then(|v| v.as_str()),
        Some("openai"),
        "Should track original primary provider"
    );
    assert_eq!(
        result
            .metadata
            ...
            .and_then(|v| v.as_str()),
        Some("anthropic"),
        "Should track fallback provider used"
    );
    assert_eq!(
        result
            .metadata
    ...
    assert_eq!(
        result
            ...
            .and_then(|v| v.as_bool()),
        Some(true),
        "Should track that transformation was lossy"
    );
    assert!(
        result.metadata.contains_key("lossy_reasons"),
